...
func TestReapExpiredSoftDeletes_PhysicalDestroy(t *testing.T) {
	...
	// 创建密钥。
	_, _, _ = mgr.CreateKey(ctx, "reap-test", seal.NewSoftwareKEK(mk), 0)
...
func TestReapExpiredSoftDeletes_NotExpiredKept(t *testing.T) {
	...
	_, _, _ = mgr.CreateKey(ctx, "keep-test", seal.NewSoftwareKEK(mk), 0)
...
func TestReapExpiredSoftDeletes_ActiveNotTouched(t *testing.T) {
	...
	_, _, _ = mgr.CreateKey(ctx, "active-reap-test", seal.NewSoftwareKEK(mk), 0)
	// 不软删除，保持 Active。
...
func TestReapExpiredSoftDeletes_MixedKeys(t *testing.T) {
	...
	_, _, _ = mgr.CreateKey(ctx, "key-expired", seal.NewSoftwareKEK(mk), 0)
	...
	// key-recent: 软删除 + 刚才。
	_, _, _ = mgr.CreateKey(ctx, "key-recent", seal.NewSoftwareKEK(mk), 0)
	_ = mgr.SoftDeleteKey(ctx, "key-recent", 1)

	// key-active: Active。
	_, _, _ = mgr.CreateKey(ctx, "key-active", seal.NewSoftwareKEK(mk), 0)
