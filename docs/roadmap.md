...
> 版本：v1.3.3 (LTS) | 日期：2026-07-01 | 维护：Yvonne 团队

本文档基于 v1.3.3 LTS 稳定版发布后的完整进度记录。

> **v1.3.3 为长期支持（LTS）版本**，作为生产部署基线，后续一段时间内不再迭代。v2.0 企业级功能规划见第六章。

## 一、版本规划总览

```
v1.0(GA) → v1.1(国密) → v1.1.1(安全) → v1.2(API) → v1.2.1(加固) → v1.2.2(签名) → v1.3.0(合规) → v1.3.1(多租户+Web) → v1.3.2(README+立规) → v1.3.3(LTS) → v2.0(企业)
 06-26     06-28       06-29          06-29       06-29          06-30         06-30         06-30              07-01            07-01        计划中
```
...
| v1.3.1 | 多租户 + Web 控制台 | 多租户隔离 + 纯 JS Web 控制台 + Admin REST API | ✅ 已发布 | 2026-06-30 |
| v1.3.2 | README 重构 + Release 立规 | README 重组 + release_gate_e2e.py + WebUI 测试立规 | ✅ 已发布 | 2026-07-01 |
| **v1.3.3** | **LTS 稳定版** | **8 个 Bug 修复 + 中英双语用户手册 + 测试隔离修复** | ✅ **已发布 (LTS)** | **2026-07-01** |
...
## 三-ter、v1.3.3 — LTS 稳定版（2026-07-01 发布）

> **长期支持（LTS）版本** — 作为生产部署基线，后续一段时间内不再迭代。v2.0 企业级功能规划见第四章。

### 3.3.1 8 个 Bug 修复（安全/并发/可用性）

| Bug | 严重度 | 文件 | 修复方案 |
|---|---|---|---|
| Bug-1 TOTP TOCTOU 并发漏洞 | HIGH | `internal/auth/totp.go` | `ValidateTOTP` 改用 `markUsed` 原子回调，依赖 DB 唯一约束 |
| Bug-2 集群通知事务原子性 | MEDIUM | `internal/lifecycle/manager.go` | `NOTIFY` 移入 `WithTx` 闭包，新增 `storage.TxNotifier` 接口 |
| Bug-3 限流器 X-Forwarded-For | HIGH | `internal/api/ratelimit.go` | `v1_router.ServeHTTP` 用 `clientIP()` + `SetTrustedProxies` 白名单 |
| Bug-4 空值缓存防穿透 | HIGH | `internal/lifecycle/cache.go` | `dekCache` 增加 negative caching（5s TTL） |
| Bug-5 分页扫描防 OOM | MEDIUM | `internal/lifecycle/manager.go` | 新增 `PagedPrefixScanner` 接口，每页 1000 条 |
| Bug-6 KEK 解密失败告警 | MEDIUM | `internal/service/core.go` | `Core` 注入 `Alerter`，失败触发 `KEKUnwrapFailure` |
| Bug-7 GDK 明文 DEK 受控暴露 | MEDIUM | `internal/service/core.go` | `plaintextDEK` 私有 + `WriteBase64To(io.Writer)` 受控方法 |
| Bug-8 TOTP uint64 溢出 | LOW | `internal/auth/totp.go` | `uint64(skew)` 改为 `int64` 计算 |

### 3.3.2 中英双语用户手册

新增 `docs/manual/` 目录：
- `user-manual.zh.md`（1633 行）— 中文用户手册
- `user-manual.en.md`（1632 行）— English user manual
- `README.md` — 手册索引
...
`internal/crypto/coverage_test.go` 和 `internal/seal/coverage_test.go` 加 `//go:build !gmsm` 和 `//go:build !hsm`，确保 stub 测试仅在无标签时运行。
